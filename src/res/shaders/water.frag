...
// "Wind Waker Ocean" by @Polyflare (29/1/15) [Modified]
// License: Creative Commons Attribution 4.0 International
// From https://www.shadertoy.com/view/ltfGD7
...
vec3 water(vec2 uv, vec3 cdir)
{
    ...
    float h = sin(uv.x + Scene.time); // Height at UV
    uv += a * h;
    h = sin(0.841471 * uv.x - 0.540302 * uv.y + Scene.time);
    uv += a * h;
#endif
    ...
    d1 = Scene.time * 0.07 + d1;
    d2 = Scene.time * 0.5 + d2;
    vec2 dist = vec2(
...
void main()
{
    FragColor = vec4(mix(water(WorldPos.xz * 20.0F, Normal), ColTilt.xyz, ColTilt.w), 1.0);
}